...
function tableToPrettyString(tb)
  ...
  function getValueStr(value, nestedCount)
      ...
      if valueType == "table" then
        return convertToStr(value, nestedCount + INDENT_OFFSET)
      end
      if valueType == "number" then
        return tostring(value)
      end
      if valueType == "string" and value:find("\n") then
        return indentMultiLineString(value, nestedCount + INDENT_OFFSET)
      end
  ...
  function convertToStr(tb, nestedCount)
        ...
        local indentStr = string.rep(" ", nestedCount + INDENT_OFFSET)
        local str = "{\n"
